...
/**
 * Actualiza nombre, apellido, carrera, graduación, cupo, LinkedIn, foto y estado activo
 * desde la planilla para tutores ya existentes.
  ...
  try {
    ...
    for (const tutor of tutors) {
      ...
      const res = await client.query(
        `WITH before AS (
           SELECT nombre, apellido, carrera, graduacion, cupo_maximo, linkedin, foto_url
           FROM tutores
           WHERE id = $8
         )
         UPDATE tutores t
         ...
         WHERE t.id = $8
           AND (
             ...
             OR before.cupo_maximo IS DISTINCT FROM $5
             OR before.linkedin IS DISTINCT FROM $6
             OR before.foto_url IS DISTINCT FROM $7
           )
         RETURNING
           ...
           (before.cupo_maximo IS DISTINCT FROM $5) AS cupo_changed,
           (before.linkedin IS DISTINCT FROM $6) AS linkedin_changed,
           (before.foto_url IS DISTINCT FROM $7) AS foto_changed`,
        [tutor.nombre, tutor.apellido, tutor.carrera, tutor.graduacion, tutor.cupo_maximo, tutor.linkedin, tutor.foto_url, existingId]
      );
