...
/**
 * Actualiza nombre, apellido, carrera, cupo, LinkedIn, foto y estado activo
  ...
  try {
    ...
    for (const tutor of tutors) {
      ...
      const res = await client.query(
        `WITH before AS (
           SELECT nombre, apellido, carrera, cupo_maximo, linkedin, foto_url
           FROM tutores
           WHERE id = $7
         )
         UPDATE tutores t
         ...
         WHERE t.id = $7
           AND (
             ...
             OR before.cupo_maximo IS DISTINCT FROM $4
             OR before.linkedin IS DISTINCT FROM $5
             OR before.foto_url IS DISTINCT FROM $6
           )
         RETURNING
           ...
           (before.cupo_maximo IS DISTINCT FROM $4) AS cupo_changed,
           (before.linkedin IS DISTINCT FROM $5) AS linkedin_changed,
           (before.foto_url IS DISTINCT FROM $6) AS foto_changed`,
        [tutor.nombre, tutor.apellido, tutor.carrera, tutor.cupo_maximo, tutor.linkedin, tutor.foto_url, existingId]
      );
